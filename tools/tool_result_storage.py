...
3. **Per-turn aggregate budget** (enforce_turn_budget): After all tool
   ...
   aggregate is under budget. Ordinary results spill to disk;
   privacy-sensitive results are omitted without a backend write.
"""
...
# Session recall may contain private user or assistant history. It is bounded
# at the tool boundary and may be sent to the model in-context, but it must
# never be copied into the active execution backend (which may be Docker, SSH,
# Modal, or another environment with different storage/retention guarantees).
_NO_SANDBOX_PERSIST_TOOL_NAMES = frozenset({"session_search"})
