...
class TestDiscoveryShape:
    ...
    def test_title_match_filters_compaction_bookends_and_caps_content(self, db):
        ...
        db.append_message(
            ...
            role="user",
            content=[
                {"type": "text", "text": "ordinary opening " + "o" * 5000}
            ],
        )
        db.append_message("s_bounded_title", role="assistant", content="ordinary response")
        db.append_message(
            "s_bounded_title",
            role="assistant",
            content={"parts": [{"text": "provider wrapper " + "d" * 50000}]},
        )
        db.append_message(
            "s_bounded_title",
            role="assistant",
        ...
        assert all(len(msg.get("content", "")) <= 1210 for msg in bookends)
        assert any(
            msg.get("content_truncated") and msg.get("original_content_chars", 0) > 1200
            for msg in bookends
        )

        assert all(len(msg.get("content", "")) <= 4010 for msg in hit["messages"])
        assert any(
            msg.get("content_truncated") and msg.get("original_content_chars", 0) > 4000
...
class TestCompactionSummaryFiltering:
    ...
    def test_compaction_summary_excluded_from_bookend_end(self, db):
        ...
        db.append_message(
            "s_compact_end",
            role="assistant",
            content=SUMMARY_PREFIX + " " + "y" * 50000,
        )
        db._conn.commit()
